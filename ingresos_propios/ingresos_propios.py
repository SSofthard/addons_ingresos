...
class registro_parque(osv.osv):
    ...
    _columns = {
        #~ 'name':fields.char(
                #~ 'Nombre',
                #~ required=True,
                #~ help="Aquí se registra el nombre del Parque",
                #~ ),
    ...
    _sql_constraints = [
        ('partner_id_uniq', 'unique (partner_id.name)', 'El periodo fiscal ya existe')
    ]
...
class genero(osv.osv):
    ...
    _columns = {
        'nombre': fields.char('Género', size=20, required=True, help='Nombre del Genero: Masculino, Femenino'),
    }
...
class reg_amb_dpto(osv.osv):
    ...
    _columns = {
        'region_id': fields.many2one('regiones', 'Región', required=True, help='Relacion de la Region con el Parque'),
...
class concesiones(osv.osv):
    ...
    def monto_a_letra2(self,cr,uid,ids,field_name,arg,context=None):
        res={}
        monto_a_letra2_obj=self.pool.get('numerosletras')
        for r in self.browse(cr,uid,ids):
            res[r.id]=monto_a_letra2_obj.convertir(r.pago_mensual)
        return res
   
    
    def monto_a_letra(self,cr,uid,ids,field_name,arg,context=None):
        res={}
        monto_a_letra_obj=self.pool.get('numerosletras')
        for r in self.browse(cr,uid,ids):
            res[r.id]=monto_a_letra_obj.convertir(r.pago_mens)
        return res
    ...
    _columns = {
        ...
        'monto_letra': fields.function(monto_a_letra,'Monto en Letra',  store=True, type="char", readonly=True, help='Monto en letra'),
        'monto_letra2': fields.function(monto_a_letra2,'Monto en Letra',  store=True, type="char", readonly=True, help='Monto en letra de U.T mensual'),
        ...
        'juridico_natural_id': fields.many2one('juridico_natural', 'Razón Social', help='Persona Natural o Juridica la cual posee la Concesion'),
        'active': fields.boolean('Activo'),
        'partner_id': fields.many2one('res.partner', 'Empresa/Persona', ondelete="cascade", required=True),
        'company_id': fields.many2one('res.company', 'Compañia',),
        'factura_por_generar_ids': fields.one2many('conc.facturas.por.generara', 'concesion_id', 'Relación Factura por Generar'),
        'documentos_ids': fields.one2many('documentos', 'juridico_natural_id', 'Documentos', help='Relacion de la Persona Natura o Juridica con los Documentos'),
        ...
        'tipo_contrato': fields.selection([
            ...
            ], 'Tipo de Contrato', help="", select=True),
    }
    _defaults = {
        'active':True,
        #~ 'isconcesion':True,
    ...
    def facturas_por_realizar(self,cr,uid,ids,fecha_inicio,fecha_fin,unidad_tributaria,pago_mensual,context=None):
        ...
        if(fecha_inicio and fecha_fin and pago_mensual):
            ...
            while i <= int(tiempo.months):
                ...
                datos.append([0,False,{'nombre': 'A Pagar: '+str(fecha_inicio2), 'pago_mensual':pago_mensual, 'fecha_facturar':fecha_inicio, 'ut_id':unidad_tributaria,  'estado': 'por_generar', 'concesion_id': False}])
                i += 1
