...
class account_tax(osv.osv):
    ...
    _columns = {
        'taxes_3_id': fields.many2many('ip.rendicion.boleto.line', 'rendicion_order_tax_3', 'taxe_id_3','rendicion_tax_id_3', 'Impuestos', readonly=True, states={'draft': [('readonly', False)]}),
    }

class rendicion_boletos(osv.Model):
    _name = "ip.rendicion.boleto"
    _inherit = ['mail.thread']
    
    def _amount_line_tax(self, cr, uid, line, context=None):
        val = 0.0
        for c in self.pool.get('account.tax').compute_all(cr, uid, line.taxe_id, line.price_unit * (1-(line.discount or 0.0)/100.0), line.product_uom_qty, line.product_id, line.rendicio_id.partner_id)['taxes']:
    ...
    def _amount_all(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            for line in order.rendicion_boleto_line:
    ...
    def _get_order(self, cr, uid, ids, context=None):
        result = {}
        for line in self.pool.get('ip.rendicion.boleto.line').browse(cr, uid, ids, context=context):
            result[line.rendicio_id.id] = True
        return result.keys()
    ...
    _columns = {
        ...
        'partner_order_id':fields.many2one('res.partner', 'Ordering Contact', readonly=True, states={'draft':[('readonly', False)]}, ),
        'partner_shipping_id':fields.many2one('res.partner', 'Delivery Address', readonly=True, states={'draft':[('readonly', False)]}, ),
        ...
        'sub_total': fields.function(_amount_all_wrapper,  string='Sub-Total',
            store={
                'ip.rendicion.boleto': (lambda self, cr, uid, ids, c={}: ids, ['rendicion_boletos_line'], 10),
                'ip.rendicion.boleto.line': (_get_order, ['price_unit', 'taxe_id', 'discount', 'product_uom_qty'], 10),
            },multi='sums', help="Sub-Total."),
        ...
        'amount_total': fields.function(_amount_all_wrapper,  string='Total',
            store={
                'ip.rendicion.boleto': (lambda self, cr, uid, ids, c={}: ids, ['rendicion_boleto_line'], 10),
                'ip.rendicion.boleto.line': (_get_order, ['price_unit', 'taxe_id', 'discount', 'product_uom_qty'], 10),
        ...
        'rendicion_boleto_line':fields.one2many('ip.rendicion.boleto.line','rendicio_id', 'Rendir Boleto', help='Detalles del boleto a rendir.'),
    ...
    def onchange_pricelist_id(self, cr, uid, ids, pricelist_id, rendicion_boleto_line, context=None):
        context = context or {}
        if not pricelist_id:
            return {}
        value = {
        ...
        if not rendicion_boleto_line or rendicion_boleto_line == [(6, 0, [])]:
            return {'value': value}
        warning = {
    ...
    def onchange_partner_id(self, cr, uid, ids, partner_id):
        ...
        return {'value':{'partner_invoice_id': addr['invoice'], 'partner_order_id':addr['contact'], 'partner_shipping_id':addr['delivery'], 'pricelist_id': pricelist,'rendicion_boleto_line':''}}
    ...
    def rendicion_terminada(self,cr,uid,ids,context=None):
        for i in self.browse(cr, uid, ids, context=context):
            for r in i.rendicion_boleto_line:
                if r.prefijo.upper():
    ...
    def rendicion_pagar(self, cr, uid, ids, context=None):
        ...
        for rendicion in self.browse(cr, uid, ids, context=context):
            ...
            account_line_vals=[]
            for line in rendicion.rendicion_boleto_line:
...
class rendicion_boleto_line(osv.Model):
    _name = "ip.rendicion.boleto.line"
    ...
    _columns = {
        ...
        'rendicio_id':fields.many2one('ip.rendicion.boleto', 'Rendir Boletos'),
        ...
        'taxe_id': fields.many2many('account.tax', 'rendicion_order_tax_2', 'rendicion_tax_id_2', 'taxe_id_2', 'Impuestos', readonly=True, states={'draft': [('readonly', False)]}),
    ...
    def create(self,cr,uid,vals,context=None):
        ...
        return super(rendicion_boleto_line, self).create(cr, uid, vals, context=context)
